...
OUTPUT_DIR = "faceid-output-deeplab-chamfer"  # 💡 Dedicated output directory to distinguish experiments
...
def main():
    ...
    print(f"\n🎯 Starting evaluation (DeepLabV3+ | Baseline: Average Chamfer Distance, With Aug)...")
    ...
    for item in pbar:
        ...
        # ==================================================
        # 🔎 Core: Baseline 2 - Minimum Average Chamfer Distance
        # ==================================================
        faceid_avg_dists = {}
        faceid_lines = defaultdict(list)

        # 1. Collect all line segments for each FaceID
        for edge in edges:
            pts = edge.get("points", [])
        ...
        # 2. Compute the average Chamfer distance for each FaceID
        for face_id, lines in faceid_lines.items():
            temp_edge_mask.fill(0)

            # Draw all line segments of this FaceID (line width of 1)
            ...
            # Extract distances at all predicted point locations and compute the average
            if len(pred_x) > 0:
                dists = dist_map[pred_y, pred_x]
                avg_dist = np.mean(dists)
                faceid_avg_dists[face_id] = avg_dist
            else:
                faceid_avg_dists[face_id] = float('inf')

        # 3. Select the FaceID with the minimum average distance as the prediction
        if faceid_avg_dists:
            pred_faceid = min(faceid_avg_dists, key=faceid_avg_dists.get)
        else:
            pred_faceid = 0
