...
OUTPUT_DIR = "faceid-output-deeplab-pure-soft"  # 💡 Dedicated output directory to distinguish experiments
...
def main():
    ...
    print(f"\n🎯 Starting evaluation (DeepLabV3+ | Ablation: Pure Soft Voting, With Aug)...")
    ...
    for item in pbar:
        ...
        # ==================================================
        # 🔎 Core: Ablation - Pure Gaussian Soft Voting (No Length Weighting)
        # ==================================================
        faceid_votes = defaultdict(float)
        ...
        for edge in edges:
            ...
            cv2.polylines(temp_edge_mask, [pts_array], isClosed=False, color=255, thickness=1)
            ...
            if len(pred_x) > 0:
                ...
                if np.any(close_mask):
                    # Retain only pure Gaussian distance decay weights
                    weights = np.exp(-(dists[close_mask] ** 2) / (2 * 1.0 ** 2))
                    faceid_votes[face_id] += np.sum(weights)
