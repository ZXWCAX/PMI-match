...
OUTPUT_DIR = "faceid-output-deeplab-trimmed"  # 💡 Dedicated output directory to distinguish experiments
...
def main():
    ...
    print(f"\n🎯 Starting evaluation (DeepLabV3+ | Ablation: Trimmed Mean Chamfer Distance)...")
    ...
    for item in pbar:
        ...
        # ==================================================
        # 🔎 Core: Ablation Study - Trimmed Mean Matching
        # ==================================================
        faceid_dists = {}
        faceid_lines = defaultdict(list)

        # 1. Collect and group all line segments by FaceID
        for edge in edges:
            pts = edge.get("points", [])
        ...
        # 2. Compute Trimmed Mean Distance
        for face_id, lines in faceid_lines.items():
            temp_edge_mask.fill(0)
            cv2.polylines(temp_edge_mask, lines, isClosed=False, color=255, thickness=1)

            inv_cand_mask = cv2.bitwise_not(temp_edge_mask)
            cand_dist_map = cv2.distanceTransform(inv_cand_mask, cv2.DIST_L2, 3)

            if len(pred_x) > 0:
                # Get distances of all predicted points to the candidate lines
                dists = cand_dist_map[pred_y, pred_x]

                # 👉 Core Ablation Logic: Sort and trim the furthest 10% outlier points
                sorted_dists = np.sort(dists)
                keep_count = max(1, int(len(sorted_dists) * 0.90))  # Keep top 90%
                trimmed_mean_dist = np.mean(sorted_dists[:keep_count])

                faceid_dists[face_id] = trimmed_mean_dist
            else:
                faceid_dists[face_id] = float('inf')

        if faceid_dists:
            pred_faceid = min(faceid_dists, key=faceid_dists.get)
        else:
            pred_faceid = 0
        ...
        for pt in corners:
            if np.all(img_left[pt[1], pt[0]] <= 30) or np.all(img_left[pt[1], pt[0]] >= 225):
                cv2.floodFill(img_left, flood_mask, pt, (240, 240, 240), (30, 30, 30), (30, 30, 30),
                              cv2.FLOODFILL_FIXED_RANGE)

        # Draw Mask Covered Area (Semi-transparent Green)
        ...
        # Overlay Model's Actual Predicted Points in Red (Semi-transparent)
        ...
        cv2.putText(img_right, f"Pred: {pred_faceid} | GT: {gt_faceid}", (30, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.5,
                    text_color, 3)

        # Concatenate and Save
        result_img = np.hstack((img_left, img_right))
        cv2_imwrite_unicode(os.path.join(OUTPUT_DIR, "correct" if is_correct else "wrong", f"{item_id}_result.png"),
