...
OUTPUT_DIR = "faceid-output-deeplab-com"  # 💡 Dedicated output directory to distinguish experiments
...
def main():
    ...
    print(f"\n🎯 Starting evaluation (DeepLabV3+ | Baseline: Center of Mass, With Aug)...")
    ...
    for item in pbar:
        ...
        # ==================================================
        # 🔎 Core: Baseline 3 - Center of Mass Nearest Neighbor (CoM-NN)
        # ==================================================
        faceid_com_dists = {}
        faceid_lines = defaultdict(list)

        # 1. Calculate global Center of Mass (CoM) of predicted point cloud
        if len(pred_x) > 0:
            center_x = int(np.clip(np.round(np.mean(pred_x)), 0, SIZE - 1))
            center_y = int(np.clip(np.round(np.mean(pred_y)), 0, SIZE - 1))
        else:
            center_x, center_y = -1, -1

        # 2. Collect all line segments for each FaceID
        for edge in edges:
            pts = edge.get("points", [])
        ...
        # 3. Calculate shortest distance from CoM to each candidate feature line
        for face_id, lines in faceid_lines.items():
            if center_x == -1:
                faceid_com_dists[face_id] = float('inf')
                continue

            temp_edge_mask.fill(0)
            cv2.polylines(temp_edge_mask, lines, isClosed=False, color=255, thickness=1)

            # Distance transform: compute distance from each pixel to the nearest line segment
            inv_mask = cv2.bitwise_not(temp_edge_mask)
            dist_map = cv2.distanceTransform(inv_mask, cv2.DIST_L2, 3)

            # Direct lookup of the distance at the CoM coordinate
            dist_to_com = dist_map[center_y, center_x]
            faceid_com_dists[face_id] = dist_to_com

        # 4. Select the FaceID closest to the CoM as the prediction
        if faceid_com_dists:
            pred_faceid = min(faceid_com_dists, key=faceid_com_dists.get)
        else:
            pred_faceid = 0
        ...
        # 💡 Special Annotation: Mark the CoM position with a red cross for paper illustration
        if center_x != -1:
            cv2.drawMarker(overlay_right, (center_x, center_y), (255, 0, 0), markerType=cv2.MARKER_CROSS, markerSize=20,
                           thickness=3)
