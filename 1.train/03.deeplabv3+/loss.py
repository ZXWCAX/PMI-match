...
class MappingLoss(nn.Module):
    def __init__(self, lambda_reg=1.0, lambda_cos=0.5, lambda_tv=0.1, eps=1e-6):
        """
        Combined loss function based on vector regression 
        (integrated with attention-weighting inside the mask).
        """
        super().__init__()
    ...
    def forward(self, pred_offsets, gt_offsets, bbox_mask):
        ...
        # ==================================================
        # 🌟 Core Enhancement: Dynamic Attention Weighting (No Gradient)
        # ==================================================
        with torch.no_grad():
            # A. Boundary-Aware Weighting
            # The L2 norm of gt_offsets represents the true distance from pixel to target edge
            distance_to_edge = torch.norm(gt_offsets, dim=1, keepdim=True)
            sigma = 20.0  # Controls decay speed; closer pixels get higher weights (up to ~3.0)
            boundary_weight = 1.0 + 2.0 * torch.exp(-distance_to_edge / sigma)

            # B. Hard-Pixel Mining
            # Larger L2 error between prediction and ground truth vectors leads to higher penalty (up to +1.0)
            ...
            # Combined dynamic weight (Shape: [B, 1, H, W])
        ...
        # Apply dynamic weights to regression loss
        ...
        # Apply dynamic weights to directional loss
